...
- Open a Terminal
  ```sh
  cd [directory that contains docker-compose.yml of your evcc]
  ```
...
- Restart the container in case there a new versuion was pulled (former container will just continue to run in case there was no upgrade).
  ```sh
  docker-compose up -d
  ```
